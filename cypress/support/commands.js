...
Cypress.Commands.add("laserSafety", () => {
    cy.get('body').then(($body) => {
        if ($body.find('[id="lasersafety_overlay"]').filter(':visible').length) {
...
Cypress.Commands.add("ignoreUpdate", () => {
    cy.wait(3000);
    cy.get('body').then(($body) => {
        if ($body.find('.ui-pnotify-container')
            .filter(':visible')
            .filter(':contains("Update Available")').length) {
            cy.get('button').filter(':contains("Ignore")').click();
        } else {
            cy.get('.ui-pnotify-container').filter(':contains("Update Available")').should('not.visible');
        }
    })
});
...
Cypress.Commands.add('loginStore', (code) => {
    cy.get('[id="design_store_iframe"]')
      ...
      .then(body => {
        cy.wrap(body) 
        if (body.find('[id="registration_modal"]').filter(':visible').length) {
            cy.iframe('[id="design_store_iframe"]').then(($button) => {
                cy.get($button).contains('Enter your existing code').click();
            });
...
Cypress.Commands.add("runWelcomeWizardIfPresent", (email, password) => {
    cy.get('body').then($body => {
        if ($body.find('[data-cy="first-run-wizard"]').filter(':visible').length > 0) {
...
Cypress.Commands.add("runloginUserIfPresent", (email, password) => {
    cy.get('body').then($body => {
        if ($body.find('[data-cy="loginscreen_dialog"]').filter(':visible').length > 0) {
...
Cypress.Commands.add("focusReminder", () => {
    cy.get('body').then($body => {
        if ($body.find('[id="laserhead_focus_reminder_modal"]').filter(':visible').length > 0) {
            cy.get('#laserhead_focus_reminder_not_again > label > input').check();
            cy.get('[id="start_job_btn_focus_reminder"]').contains("It's focused!").click({force:true});
        } else {
            cy.get('[id="laserhead_focus_reminder_modal"]').filter(':visible').should('not.exist')
        }
    });
})
