describe("Find Mr Beam", function () {
    ...
    it("find.mrbeam.org", function () {
        cy.get('[id="settings_plugin_findmymrbeam_link"]')
            .if("exist")
            .then(() => {
                cy.get('[id="settings_plugin_findmymrbeam_link"]').click();
                cy.get(
                    '[data-bind="visible: settings.settings.plugins.findmymrbeam.enabled() && registered()"] > a'
                )
                    .invoke("attr", "href")
                    .then((myLink) => {
    ...
    it("Privacy policy link", function () {
        cy.get('[id="settings_plugin_findmymrbeam_link"]')
            ...
            .then(() => {
                cy.get('[id="settings_plugin_findmymrbeam_link"]').click();
                cy.get(
                    "#settings_findmymrbeam > .form-horizontal > .control-group > :nth-child(1) > p > a"
                )
                    .invoke("attr", "href")
                    .then((myLink) => {
                        cy.request(myLink).then((resp) => {
    ...
    it("Mr Beam Status Light tells you how to connect", function () {
        cy.get('[id="settings_plugin_findmymrbeam_link"]')
            ...
            .then(() => {
                cy.get('[id="settings_plugin_findmymrbeam_link"]').click();
                cy.get(":nth-child(3) > :nth-child(5) > a")
                    .invoke("attr", "href")
                    .then((myLink) => {
                        cy.request(myLink).then((resp) => {
                            expect(resp.status).to.eq(200);
