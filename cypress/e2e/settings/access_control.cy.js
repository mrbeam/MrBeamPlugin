describe("Access control", function () {
    ...
    it("Access control - new user", function () {
        ...
        cy.get('.settings_users_name').contains("dev+1@mr-beam.org").should('to.exist')
        cy.logout();
        ...
        cy.wait(1000);
        cy.laserSafety()
        cy.get('[id="navbar_login"]').invoke('prop', "innerText").should('to.contain', "dev+1@mr-beam.org");
    });
    ...
    it("Access control - delete user", function () {
        ...
        cy.get('.settings_users_name').contains("dev+1@mr-beam.org").should('not.exist')
    });

    it.only("Access control abort button", function () {
        cy.get('[id="settings_users_link"]').click();
        ...
        cy.get(".fa-key").click();
        cy.get('[id="settings-usersDialogChangePassword"]').should('to.visible')
        cy.get(
            '#settings-usersDialogChangePassword > .modal-footer > [data-dismiss="modal"]'
        ).click();
        cy.get('[id="settings-usersDialogChangePassword"]').should('not.visible')
    });
