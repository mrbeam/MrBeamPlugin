describe("Login", function() {
    ...
    it('Select/Unselect', function(){
      cy.get('.files_template_model_svg').first().realHover()
        .find('.selection_box').first().click({force:true});
      cy.wait(3000);
    ...
    it('Upload designs by clicking', function(){
      ...
      cy.wait(5000)
      cy.get('.files_template_model_svg').first().should('to.exist','test.svg');
      cy.logout();
    });

    it('Delete designs by burger menu', function(){
      cy.get('.files_template_model_svg').first().should('contain', 'test.svg').find('.icon-reorder').click()
      cy.get('.icon-trash:visible').click();
      cy.get('.files_template_model_svg').first().should('not.exist','test.svg');
      cy.logout();
    });
    ...
    it('Add folder', function(){
      ...
      cy.wait(7000);
      cy.get('.file_list_entry > .title').contains('test folder').should('to.exist');
      cy.logout();
    });
    ...
    it('Delete folder by burger menu', function(){
      cy.get('#gcode_file_23e36bc802346e6e128d3370e49fc959 > .file_list_entry > .title > .dropdown-toggle > .icon-reorder').first().click();
      cy.get('.dropdown-menu')
        .get('.icon-trash:visible').first().click();
      cy.get('.file_list_entry > .title').first().should('not.exist' , 'test folder');
      cy.logout();
    });

    it('Download designs by burger menu', function(){
      cy.intercept('GET', 'http://localhost:5002/downloads/files/local/Test_plan_1.svg').as('downloadFile');
      cy.get('.files_template_model_svg').first().find('.icon-reorder').click()
      cy.wait(1000)
