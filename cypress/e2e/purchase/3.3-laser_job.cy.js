describe("Laser Job", function () {
    ...
    it('Add design', function () {
        ...
        cy.get('[id="files_list"]').then($elem => {
            if (($elem.find('.files_template_model_image').filter(':contains("paris2.jpg")').length)) {}
            else {
                const filepath = 'paris2.jpg';
                cy.get('.fileinput-button input[type="file"]').attachFile(filepath);
                cy.wait(5000)
                cy.get('.files_template_model_image').contains('paris2.jpg').should('to.exist');
            };
        });
        cy.get('.files_template_model_image').filter(':contains("paris2.jpg")').click();
        cy.wait(3000);
        cy.get('.userIMG').click({force:true});
        cy.get('[id="translateHandle"]').move({ deltaX: 213.9689, deltaY: -144.1241, force:true});
