describe("Laser Job", function () {
    ...
    it('Add design dxf', function () {
        ...
        cy.get('[id="files_list"]').then($elem => {
            if (($elem.find('.files_template_model_dxf').filter(':contains("paris1.dxf")').length)) {}
            else {
                const filepath = 'paris1.dxf';
                cy.get('.fileinput-button input[type="file"]').attachFile(filepath);
                cy.wait(5000)
                cy.get('.files_template_model_dxf').contains('paris1.dxf').should('to.exist');
            };
        });
        cy.get('.files_template_model_dxf').filter(':contains("paris1.dxf")').click();
        cy.wait(3000);
