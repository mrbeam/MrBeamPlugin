...
from octoprint_mrbeam.model.document_model import (
    DocumentLinkModel,
    DocumentModel,
    DocumentSimpleModel,
)
...
ALL_EXEMPTED = "*"
EXEMPTED_TRANSLATIONS = {"en": {ALL_EXEMPTED}, "de": {"Quickstart Guide"}}
...
class DocumentService:
    ...
    def get_documents_for(self, definition):
        ...
        document_links = [
            DocumentLinkModel(
                language, self._get_url_for_definition_language(definition, language)
            )
    ...
    def get_document_simple_for(self, definition, language):
        ...
        """
        document_link = DocumentLinkModel(
            language, self._get_url_for_definition_language(definition, language)
        )
        title_translated = self._get_title_translated(definition)
    ...
    def _get_title_translated(self, definition):
        title_key = string_util.separate_camelcase_words(
            definition.mrbeamdoc_type.value
        )
    ...
    def _get_url_for_definition_language(definition, language, extension="pdf"):
        return (
            "/plugin/mrbeam/docs/%(mrbeam_model)s/%(language)s/%(mrbeam_type)s.%(extension)s"
            % {
    ...
    def _log_error_on_missing_translation(self, translation_key, translation):
        ...
        exempted_translations_for_language = EXEMPTED_TRANSLATIONS.get(
            get_locale().language, {}
        )
        is_not_exempted_key = not any(
            key in exempted_translations_for_language
            for key in [ALL_EXEMPTED, translation_key]
        )
        ...
        if is_not_exempted_key and translation_key == translation:
            self._logger.error(
                ...
                % {"translation_key": translation_key, "translation": translation}
            )
