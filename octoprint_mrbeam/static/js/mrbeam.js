...
browser.is_supported =
  (browser.is_chrome && browser.chrome_version >= 60) ||
  browser.is_ipad ||
  browser.is_firefox;
    ...
    self.onDataUpdaterPluginMessage = function (plugin, data) {
      ...
      if ("frontend_notification" in data) {
        ...
        new PNotify({
          ...
          hide: !(
            notification["hide"] == false || notification["sticky"] == true
          ),
          delay: delay,
        });
      }
          ...
          if (modalElement.length !== 0) {
            modalElement.each(function () {
              if (!$(this)[0].hasChildNodes() && modalElement.length === 1) {
                ...
                $(this)[0].hasChildNodes() &&
                modalElement.length === 1 &&
                $(this).find(".modal.hide.fade").inlineStyle("display") ===
                  "none"
              ) {
                setTimeout(() => {
                  if (
                    $(this).find(".modal.hide.fade").hasClass("modal") &&
                    $(this).find(".modal.hide.fade").inlineStyle("display") ===
                      "none"
                  ) {
