...
Snap.plugin(function (Snap, Element, Paper, global) {
	...
	Element.prototype.separate_by_native_elements = function (max_parts=0) {
		...
		let buckets = max_parts > 0 ? Math.min(native_elements.length, max_parts) : native_elements.length;
		if(buckets <= 1) {
	 ...
	 * @returns {array} svg snippets (empty means not separatable)
	 */
	Element.prototype.separate_by_non_intersecting_bbox = function (cancel=null, progress_cb=null) {
		const old_element = this;
		let native_elements = old_element.get_native_elements();
		...
		for (let i = 0; i < native_elements.length; i++) {
			...
			for (let j = 0; j < by_bbox.length; j++) {
				if(cancel && typeof cancel === "function" && cancel()) return [];
			...
			if(progress_cb && typeof progress_cb === "function") progress_cb(progress_counter / progress_total);
		}
