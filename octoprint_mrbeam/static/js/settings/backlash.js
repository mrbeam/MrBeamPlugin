...
$(function () {
	function BacklashViewModel(parameters) {
		...
		self.backlash_compensation_x = ko.observable(self.backlash_compensation_x);
		self.backlash_compensation_x.extend({ rateLimit: { timeout: 500, method: "notifyWhenChangesStop" } });
		self.backlash_compensation_x.subscribe(function(val){
			self.settings.settings.plugins.mrbeam.machine.backlash_compensation_x(val);
			self.settings.saveData(undefined, function(newSettings){
				console.log("Saved backlash compensation x", newSettings.plugins.mrbeam.machine.backlash_compensation_x);
			});
		});
		...
		self.engrave_calibration_pattern = function(){
			...
			OctoPrint.simpleApiCommand("mrbeam", "generate_backlash_compenation_pattern_gcode", data)
					.done(function(response){
						let file = {path: response.calibration_pattern, origin: response.target};
