...
$(function () {
    function CustomMaterialViewModel(parameters) {
        ...
        $('#settings_mrbeam_custom_material_restore_input').on('change', function (ev) {
            ...
            self.readLocalFile(ev, function(data){
                ...
                if (data && 'custom_materials' in data) {
                    ...
                    self.conversion.restore_material_settings(data.custom_materials);
                } else {
                    console.error("Unable to load custom material settings from file.");
					new PNotify({
                        title: gettext("Failed to load file"),
                        text: _.sprintf(gettext("Unable to load custom material settings from given file. Please make sure that it is a Mr Beam Custom Material Settings backup file.")),
                        type: "error",
                        hide: true
                    });
                }
        ...
        self.backupDataUrl = ko.computed(function () {
            ...
            return "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(payload));
        });
