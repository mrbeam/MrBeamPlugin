var Color = (function (window) {
    ...
    var isNamedColor = function (key) {
        ...
        return namedColors.hasOwnProperty(lc) ? namedColors[lc] : null;
    };
            ...
            default:
                switch (typeof value) {
                    case "object":
                        ...
                        return this;
                    case "string":
                        value = namedColors.hasOwnProperty(value)
                            ? namedColors[value]
                            : value;
                        switch (true) {
    ...
    Color.prototype.interpolate = function (destination, factor) {
        ...
        this._red = absround(
            +this._red + (destination._red - this._red) * factor
        );
        this._green = absround(
            +this._green + (destination._green - this._green) * factor
        );
        this._blue = absround(
            +this._blue + (destination._blue - this._blue) * factor
        );
        this._alpha = absround(
            +this._alpha + (destination._alpha - this._alpha) * factor
        );
    ...
    Color.prototype._RGB2INT = function () {
        this._decimal =
            (this._red << 16) | ((this._green << 8) & 0xffff) | this._blue;
    };
    ...
    Color.prototype.getPRGB = function () {
        var components = [
            absround((100 * this._red) / 255) + "%",
            absround((100 * this._green) / 255) + "%",
            absround((100 * this._blue) / 255) + "%",
        ];
        return "rgb(" + components.join(", ") + ")";
    };
    /**
    ...
    Color.prototype.getHSL = function () {
        var components = [
            absround(this._hue),
            absround(this._saturation) + "%",
            absround(this._lightness) + "%",
        ];
