    ...
    function VectorConversionViewModel(params) {
            ...
            if (self.user_materials_enabled) {
                ...
                OctoPrint.simpleApiCommand("mrbeam", "custom_materials", {})
                    .done(function (response) {
                        self._update_custom_materials(
                            response.custom_materials
                        );
                    })
                    .fail(function () {
            ...
            if (self.user_materials_enabled) {
                if (!self.selected_material().custom) {
                    self.save_custom_material_name(
                        gettext("My") + " " + self.selected_material().name
                    );
        ...
        self.delete_material = function (m, event) {
            ...
            OctoPrint.simpleApiCommand("mrbeam", "custom_materials", postData)
                ...
                .fail(function () {
                    console.error(
                        "unable to delete custom material:",
                        postData
                    );
                });
        };
        ...
        self.save_material_settings = function () {
            ...
            for (var i = 0; i < vectors.length; i++) {
                // heuristics: assuming that the strongest of all vector jobs is the cutting one.
                var v = vectors[i];
                if (!v.engrave) {
            ...
            if (self.custom_materials()[key]) {
                ...
                new_material.description = $("<div>")
                    .html(self.save_custom_material_description())
                    .text();
            ...
            if (new_material.colors[color]) {
                for (
                    var t = 0;
                    t < new_material.colors[color].cut.length;
                    t++
                ) {
                    ...
                    if (
                        ...
                        item.thicknessMM > 0 &&
                        (cut_setting == null ||
                            item.thicknessMM !== cut_setting.thicknessMM)
                    ) {
                        tmp.push(item);
                    }
                }
        ...
        self.restore_material_settings = function (materials) {
            ...
            OctoPrint.simpleApiCommand("mrbeam", "custom_materials", postData)
                ...
                .fail(function () {
                    console.error(
                        "Unable to restore custom material settings: ",
                        postData
                    );
                    new PNotify({
                        title: gettext("Error while saving settings!"),
        ...
        self._save_material_load_local_image = function (img_file) {
            ...
            loadImage(
                img_file,
                self._on_save_material_local_image_loaded,
                options
            );
        };
        ...
        self.show_save_custom_material_reset_button = ko.computed(function () {
            return (
                self.save_custom_material_image() &&
                self.save_custom_material_image().length > 0
            );
        });
        ...
        self._update_custom_materials = function (list) {
            ...
            console.log(
                "Loaded custom material settings: ",
        ...
        self.get_closest_color_params = function () {
            ...
            if (material !== null && hex !== null) {
                ...
                var closest = self._find_closest_color_to(
                    hex,
                    available_colors
                );
        ...
        self._set_available_material_thicknesses = function (material, color) {
            if (material !== null && color !== null) {
                ...
                if (material.colors[color].engrave !== null) {
                    available_thickness = available_thickness.concat(
                        self.engrave_only_thickness
                    );
                }
        ...
        self._getColorIcon = function (color) {
            var i = $("<div />", {
                ...
                title: gettext(
                    "Drag and drop this box in the Skip area if you want to skip cutting this color. Drop it in Engrave area if you just want to engrave it, or alternatively create a new cutting job with different parameters by dropping it in the bottom section."
                ),
            }).on({
                dragstart: function (ev) {
                    $(ev.target).tooltip("hide");
                    window.mrbeam.colorDragging.colorDragStart(
                        ev.originalEvent
                    );
                },
                dragend: function (ev) {
                    window.mrbeam.colorDragging.colorDragEnd(ev.originalEvent);
                },
            });
        ...
        self.apply_vector_proposal = function () {
            ...
            for (var i = 0; i < vector_jobs.length; i++) {
                ...
                $(job)
                    ...
                    .prop("checked", p.progressive);
                $(job)
                    .find(".param_piercetime")
                    .val(p.cut_pierce || 0);
                $(job)
        ...
        // see util.js
        self.imgIntensityWhite = observableInt(
            self,
            self.JOB_PARAMS.default.intensityWhite
        );
        self.imgIntensityBlack = observableInt(
            self,
            self.JOB_PARAMS.default.intensityBlack
        );
        self.imgFeedrateWhite = observableInt(
            self,
            self.JOB_PARAMS.default.feedrateWhite
        );
        self.imgFeedrateBlack = observableInt(
            self,
        ...
        self.get_color_assignment_required = function () {
            ...
            var assigned_images =
                $("#engrave_job .assigned_colors").children().length > 0;
            var rasters_present = self.show_image_parameters();
            return (
                (vectors_present && vec.length === 0) ||
        ...
        self.show_conversion_dialog = function () {
            ...
            if (self.show_vector_parameters() || self.show_image_parameters()) {
                ...
                var gcodeFile = self.create_gcode_filename(
                    self.workingArea.placedDesigns()
                );
        ...
        self.get_current_multicolor_settings = function (
            prepareForBackend = false
        ) {
            var data = [];

            var intensity_black_user = self.imgIntensityBlack();
            var intensity_white_user = self.imgIntensityWhite();
            var speed_black = self.imgFeedrateBlack();
            var speed_white = self.imgFeedrateWhite();

            // vector icons dragged into engraving.
            $(
                "#engrave_job_drop_zone_conversion_dialog>.cutting_job_color"
            ).each(function (i, el) {
                ...
                if (
                    ...
                    if (
                        $("#engrave_job_drop_zone_conversion_dialog").find(
                            ".cutting_job_color"
                        ).length > 0
                    ) {
        ...
        self._isValidVectorSetting = function (
            intensity,
            feedrate,
            passes,
            pierceTime
        ...
        self.get_design_files_info = function () {
            ...
            for (let i = 0; i < placedDesigns.length; i++) {
                ...
                let dim_x = $("#" + currentDesign.id)
                    .find(".horizontal")
                    .val();
                let dim_y = $("#" + currentDesign.id)
                    .find(".vertical")
                    .val();
                ...
                if (format === "image") {
                    let file_name = $("#" + currentDesign.id)
                        .find(".title")
                        .text();
        ...
        self.is_advanced_settings_checked = function () {
            const advancedSettingsCb = $(
                "#parameter_assignment_show_advanced_settings_cb"
            );
        ...
        self._allJobsSkipped = function () {
            ...
            if ($("#no_job .color_drop_zone").children().length > 0) {
                ...
                if (
                    $("#engrave_job .color_drop_zone").children(":visible")
                        .length > 0
                ) {
                    allSkipped = false;
                } else {
        ...
        self._validJobForMaterial = function () {
            ...
            for (let i = 0; i < vector_jobs.length; i++) {
                ...
                if (
                    self.has_cutting_proposal() &&
                    colorDrops.children().length > 0
                ) {
                    validCut = true;
                }
            }
        ...
        self.moveJobsToEngravingColorDefaultOption = function (color) {
            ...
            if (!self.engraveOnlyForced) {
                ...
                if (
                    color in material.colors &&
                    material.colors[color].cut.length > 0
                ) {
                    hasCut = true;
                }
        ...
        self.forceEngraveOnly = function () {
            ...
            for (let i = 0; i < vector_jobs.length; i++) {
                ...
                for (let j = 0; j < numJobs; j++) {
                    ...
                    $("#engrave_job > .span3 > .color_drop_zone").append(
                        moveCut
                    );
        ...
        self.undoForceEngraveOnly = function () {
            ...
            let engraving_jobs = $("#engrave_job .color_drop_zone").children(
                ":visible"
            );
        ...
        self._allParametersSet = function () {
            ...
            if (
                self.has_engraving_proposal() &&
                $("#engrave_job .color_drop_zone").children().length > 0
            ) {
                if (self.imgIntensityWhite() === "") {
                    self._missing_parameter_hint(
                        $("#svgtogcode_img_intensity_white")
                    );
                    allSet = false;
                }
                if (self.imgIntensityBlack() === "") {
                    self._missing_parameter_hint(
                        $("#svgtogcode_img_intensity_black")
                    );
                    allSet = false;
                }
                if (self.imgFeedrateWhite() === "") {
                    self._missing_parameter_hint(
                        $("#svgtogcode_img_feedrate_white")
                    );
                    allSet = false;
                }
                if (self.imgFeedrateBlack() === "") {
                    self._missing_parameter_hint(
                        $("#svgtogcode_img_feedrate_black")
                    );
                    allSet = false;
                }
            }
            ...
            if (self.dontRemindMeAgainChecked() == self.showFocusReminder()) {
                ...
                OctoPrint.simpleApiCommand("mrbeam", "focus_reminder", data)
                    .done(function (response) {
                        ...
                        console.log(
                            "simpleApiCall response for saving focus reminder state: ",
                            response
                        );
                    })
        ...
        self.convert = async function () {
            if (
                ...
                self.files.startGcodeWithSafetyWarning(
                    self.gcodeFilesToAppend[0]
                );
            } else if (self._allJobsSkipped()) {
                const message = gettext(
                    "There is nothing to laser, all jobs are set to be skipped."
                );
                ...
                if (self._allParametersSet()) {
                    ...
                    var data = {
                        ...
                        job_time_estimation_v2:
                            self.estimated_job_time().val.total.sum.val, // estimation in s
                    };
                    ...
                    new PNotify({
                        ...
                        text: gettext(
                            "Unable to start conversion because a parameter is missing."
                        ),
                        type: "warn",
                        hide: true,
                    });
        ...
        self.onStartup = function () {
            ...
            $("#customMaterial_colorPicker").bind(
                "change",
                self.save_custom_material_color
            );
        ...
        self.onAllBound = function () {
            self.hasCompressor(
                self.settings.settings.plugins.mrbeam.hw_features.has_compressor()
            );
        ...
        self.onSlicingProgress = function (
            slicer,
            model_path,
            machinecode_path,
            progress
        ...
        self.onEventSlicingFailed = function (payload) {
            ...
            if (
                ...
                payload["reason"].startsWith("OutOfSpaceException")
            ) {
                var html = "<ul>";
                html +=
                    "<lh>" +
                    gettext(
                        "To free up some disk space you may want to perform one or all of the following suggestions:"
                    ) +
                    "</lh>";
                html +=
                    "<li>" +
                    gettext(
                        "Delete GCode files: Go to design library and click 'Only show GCode files' on the left. Here you can delete files from the according context menu."
                    ) +
                    "</li>";
                html +=
                    "<li>" +
                ...
                new PNotify({
                    title: gettext("Get more free disk space"),
                    text: html,
                    type: "info",
                    hide: false,
                });
            ...
            for (var i = 0; i < engrave_items.length; i++) {
                ...
                if (id !== "cd_engraving") {
                    ...
                    if ($("#" + slider_id + "_out").length > 0) {
                        ...
                        let color_circle =
                            '<div class="vector_mapping_color_circle" style="background:' +
                            hex +
                            '"/></div>';
                        let slider =
                            '<input id="' +
                            slider_id +
                            '" class="svgtogcode_grayscale mrb_slider conversion_range_slider" type="range" min="0" max="255" value="' +
                            val +
                            '" /></div>';
...
window.mrbeam.colorDragging = {
    ...
    colorDragEnd: function (ev) {
        ...
        setTimeout(function () {
            $("body").removeClass(
                "colorDragInProgress vectorDrag engravingDrag"
            );
        }, 200);
        $(".color_drop_zone, .img_drop_zone").removeClass("hover");
    },
