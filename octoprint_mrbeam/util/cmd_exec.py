...
def exec_cmd_output(cmd, log=True, shell=False, loglvl=DEBUG):
	...
	try:
		...
		if not log:
			...
			output = e.output
		_logger.log(loglvl, "Failed to execute command '%s', return code: %s, output: '%s'", cmd, e.returncode, output)

	except Exception as e:
		code = 99
		output = "{e}: {o}".format(e=e, o=output)
		_logger.log(loglvl,"Failed to execute command '%s', return code: %s, output: '%s'", cmd, None, output)
