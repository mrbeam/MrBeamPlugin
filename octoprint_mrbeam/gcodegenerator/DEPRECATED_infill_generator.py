...
class InfillGenerator():
	...
	def __init__( self , doc, nodeList):
	...
	def recursivelyTraverseSvg( self, aNodeList,
		...
		for node in aNodeList:
			...
			if node.tag == addNS( 'g', 'svg' ) or node.tag == 'g':
			...
			elif node.tag == addNS( 'ellipse', 'svg' ) or \
				node.tag == 'ellipse' or \
				node.tag == addNS( 'circle', 'svg' ) or \
				node.tag == 'circle':
			...
			elif node.tag == addNS( 'eggbot', 'svg' ) or node.tag == 'eggbot':

				pass

			elif node.tag == addNS( 'text', 'svg' ) or node.tag == 'text':

				print( 'Warning: unable to draw text, please convert it to a path first.' )
				...
				print( 'Warning: unable to draw object <%s>, please convert it to a path first.' % node.tag )
				pass

	def joinFillsWithNode ( self, node, stroke_width, color, path ):
		...
		#g = etree.SubElement( parent, addNS( 'g', 'svg' ) )
		...
		style = { 'stroke': color, 'fill': 'none', 'stroke-width': '%f' % stroke_width}
