#!/usr/bin/env python
"""simplepath.py functions for digesting paths into a simple list structure
Copyright (C) 2005 Aaron Spike, aaron@ekips.org This program is free software;
...
def lexPath(d):
    """returns and iterator that breaks path data identifies command and
    parameter tokens."""
    offset = 0
    length = len(d)
    delim = re.compile(r"[ \t\r\n,]+")
...
def parsePath(d):
    ...
    Removes all shorthand notation. Converts coordinates to absolute.
    """
    retval = []
    lexer = lexPath(d)
    pen = (0.0, 0.0)
