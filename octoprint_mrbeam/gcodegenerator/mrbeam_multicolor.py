...
###
Laserengraver_current_version = "0.02"
...
################################################################################
###		print prints any arguments into specified log file
################################################################################

def print_(*arg):
	if(option_dict['log_filename'] != ''):
		f = open(option_dict['log_filename'],"a")
		for s in arg :
		...
		msg = ""
		for s in arg :
			msg += str(unicode(s).encode('unicode_escape'))+", "
		print msg + "\n"
...
#					self._log.debug((-f(w[k][i-1]),-f(w[k][i]), [i1[5] for i1 in l1]) )
...
#			self.options['file'] = filename
		...
		self._log.info("Testing writing rights on '%s'"%(self.options['directory']+self.options['file']))
		try:	 
			f = open(self.options['directory']+self.options['file'], "w")	
			f.close()							
		except:
...
################################################################################
	def generate_gcode_color(self, curve, color='#000000', pierce_time=0):
		self._log.info( "generate_gcode_Color()")
	...
	def transform(self, source_point, layer, reverse=False):
		...
		if layer not in self.transform_matrix:
			...
			self._log.debug(str(self.layers))
			self._log.debug(str("I: " + str(i)))
			self._log.debug("Transform: " + str(self.layers[i]))
			if self.layers[i] not in self.orientation_points :
				...
				if len(points)==3:
					self._log.debug("Layer '%s' Orientation points: " % orientation_layer.get(inkex.addNS('label','inkscape')))
					for point in points:
			...
			self._log.debug(self.transform_matrix)
			self._log.debug(self.transform_matrix_reverse)
			#self._log.debug("scalematrix", self.transform_matrix)
			#self._log.debug("revmatrix", self.transform_matrix_reverse)
			...
			self._log.debug("Z automatic scale = %s (computed according orientation points)" % self.Zauto_scale[layer])
		...
		def recursive(g) :
			for i in g:
				...
				if i.tag ==inkex.addNS("g",'svg') :
					recursive(i)
		self._log.info("get_defs %s" % self.defs)
...
################################################################################
	def get_info(self):
		...
		def recursive_search(g, layer):
			...
			items.reverse()
			if(len(items) > 0):
				self._log.info("recursive search: %i - %s"  %(len(items), g.get("id")))
			for i in items:
				if i.tag == inkex.addNS("g",'svg') and i.get(inkex.addNS('groupmode','inkscape')) == 'layer':
					...
					if points != None :
						...
						self._log.info("Found orientation points in '%s' layer: %s" % (layer.get(inkex.addNS('label','inkscape')), points))
					else :
					...
					# group
						...
						self._log.info("ignoring not supported tag %s" % i.tag)
						self._log.debug("ignoring not supported tag: %s \n %s \n\n" % (i.tag, inkex.etree.tostring(i)))
					
		recursive_search(self.document.getroot(),self.document.getroot())
		self._log.info("self.layers: %i" % len(self.layers))
		#self._log.info("self.selected_paths", len(self.selected_paths)
...
#						if r!=None:
#							self._log.info("got path=",r.group(1))
...
###
################################################################################
	def set_laser_params(self, paramArray = []):
		if len(paramArray) > 0:
			for job in paramArray:
				#job = {"job":1,"color":"#666666","intensity":"625","feedrate":"400","piercetime":"0","passes":"0"}
				self.colorSettings[job['color']] = job
				
		self._log.info("set_laser_params() %s " % self.colorSettings)
	...
	def laser(self, on_progress=None, on_progress_args=None, on_progress_kwargs=None) :
		...
		self.set_laser_params(self.options['multicolor'])
		
		with open("/tmp/debug.txt", "w") as text_file:
			text_file.write("Multicolor_options: %s" % self.options)
		...
		#self._log.info("processing ", len(self.layers), " layers")
		# sum up
		itemAmount = 1
		for layer in self.layers :
			if layer in paths :
		...
		for layer in self.layers :
			if layer in paths :
				...
				for colorKey in curvesD.keys():
					settings = self.colorSettings.get(colorKey, {'intensity': -1, 'feedrate': -1, 'passes': 0})
					...
					curveGCode = self.generate_gcode(curvesD[colorKey],
													int(settings['intensity']),
													int(settings['feedrate']),
									pierce_time)
...
################################################################################
	def orientation(self, layer=None) :
		self._log.info("entering orientations")
		if layer == None :
			layer = self.current_layer if self.current_layer is not None else self.document.getroot()
		...
		if self.options['unit'] == "G21 (All units in mm)" : 
			...
			self._log.info("orientation_scale < 0 ===> switching to mm units=%0.10f"%orientation_scale )
		elif self.options['unit'] == "G20 (All units in inches)" :
			points = [[0.,0.,0.],[5.,0.,0.],[0.,5.,0.]]
			orientation_scale = self.options['svgDPI'] / viewBoxScale
			self._log.info("orientation_scale < 0 ===> switching to inches units=%0.10f"%orientation_scale )
...
if __name__ == "__main__":
	...
	debug_multicolor = [
		{"passes": "1", "feedrate": "1000", "piercetime": "0", "color": "#666666", "intensity": "1000", "job": 1}, 
		{"passes": "1", "feedrate": "1000", "piercetime": "0", "color": "#d40000", "intensity": "1000", "job": 1}, 
		{"passes": "1", "feedrate": "400", "piercetime": "0", "color": "#00ffff", "intensity": "625", "job": 2}
	]
	
	option_dict['multicolor'] = debug_multicolor
