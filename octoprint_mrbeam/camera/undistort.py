...
def prepareImage(
    ...
    if type(input_image) is str:
        ...
        if not isfile(input_image):
            no_Image_error_String = "Could not find a picture under path: <{}>".format(
                input_image
            )
    ...
    workspaceCorners = {
        qd: markers[qd]
        - _pic_settings[CALIB_MARKERS_KEY][qd][::-1]
        + _pic_settings[CORNERS_KEY][qd][::-1]
        for qd in QD_KEYS
    }
    logger.debug(
        "Workspace corners \nNW % 14s  NE % 14s\nSW % 14s  SE % 14s"
    ...
    if debug_out:
        save_debug_img(_debug_drawCorners(img, workspaceCorners), "drawcorners")

    # warp image
    warpedImg = _warpImgByCorners(img, workspaceCorners, zoomed_out)
    if debug_out:
        save_debug_img(warpedImg, "colorwarp")
    ...
    if retval == SUCCESS_WRITE_RETVAL:
        savedPics["cropped"] = True
        ...
        if isMarkerMask(spot[start[0] : stop[0], start[1] : stop[1]]):
            ...
            if HUE_BAND_LB <= avg_hsv[0] <= 180 or 0 <= avg_hsv[0] <= HUE_BAND_UB:
                ...
                debug_roi = cv2.drawMarker(
                    cv2.cvtColor(
                        cv2.bitwise_or(threshOtsuMask, gaussianMask), cv2.COLOR_GRAY2BGR
                    ),
                    (x, y),
                    (0, 0, 255),
                    cv2.MARKER_CROSS,
