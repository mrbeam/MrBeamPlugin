...
class LoopThread(threading.Thread):
    ...
    def __init__(self, target, stopFlag, args=(), kwargs=None):
        ...
        self._logger = mrb_logger(
            "octoprint.plugins.mrbeam.loopthread", lvl=logging.INFO
        )
        self.ret = None
        self.t = target
            ...
            try:
                ...
                if connectionErrMsg in str(e):
                    self._logger.warning(
                        "Camera was not ready yet, it should restart by itself."
                    )
                else:
                    raise e
            except Exception as e:
                self._logger.error(
                    "Handled exception in picamera: %s, %s", e.__class__.__name__, e
                )
                raise
            self.running.clear()
    ...
    def async_next(self, **kw):
        self._logger.debug(
            "captureLoop running %s, stopFlag %s",
            self.running.isSet(),
            self.stopFlag.isSet(),
        )
        time.sleep(0.1)
...
class MrbCamera(PiCamera, Camera):
    def __init__(self, worker, stopEvent=None, shutter_speed=None, *args, **kwargs):
        ...
        # self.exposure_mode = ''
        self.stopEvent = (
            stopEvent or threading.Event()
        )  # creates an unset event if not given
        self.start_preview()
        self._logger = mrb_logger(
