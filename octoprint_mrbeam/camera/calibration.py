...
class BoardDetectorDaemon(Thread):
    ...
    def __init__(
        ...
        Thread.__init__(
            self, target=self.processInputImages, name=self.__class__.__name__
        )
    ...
    def processInputImages(self):
        ...
        while not self._stop.is_set():
            ...
            self.state.refresh(
                imgFoundCallback=self.event_bus.fire,
            ...
            if (
                ...
                or self.startCalibrationWhenIdle
            ...
            if len(runningProcs.keys()) < self.procs.value and self.state.getPending():
                ...
                self._logger.debug(
                    ...
                    % (len(runningProcs.keys()), self.procs.value, path)
                )
            ...
            if (
                lensCalibrationProc
                and lensCalibrationProc.exitcode is not None
                and lensCalibrationProc.exitcode != 0
            ):
                self._logger.warning(
                    "Something went wrong with the lens calibration process"
                )
...
class calibrationState(dict):
    def __init__(
        self,
        imageSize=camera.LEGACY_STILL_RES,
        changeCallback=None,
        npzPath=None,
        rawImgLock=None,
        *args,
    ...
    def getAllPending(self):
        return list(
            map(
                lambda elm: elm[0],
                filter(lambda elm: elm[1]["state"] == STATE_QUEUED, self.items()),
            )
        )
