...
class TestBurgerMenuService(TestCase):
    ...
    def test_get_burger_menu_model__with_none__should_return_empty_burger_menu_model(
        self,
    ):
    ...
    @patch(
        "octoprint_mrbeam.services.burger_menu_service.MrBeamDocUtils.get_mrbeam_definitions_for"
    )
    @patch("octoprint_mrbeam.services.burger_menu_service.get_locale")
    def test_get_burger_menu_model__with_language_not_valid_for_definition__should_fallback_to_english(
        self, get_locale_mock, get_mrbeam_definitions_for_mock
    ):
        get_locale_mock.return_value = MagicMock(language="de")
        MOCK_DEFINITION = MrBeamDocDefinition(
    ...
    @patch("octoprint_mrbeam.services.burger_menu_service.get_locale")
    def test_get_burger_menu_model__with_unicode_translation__should_work(
        self, get_locale_mock, get_text_mock, get_mrbeam_definitions_for_mock
    ):
        get_locale_mock.return_value = MagicMock(language="es")
        get_text_mock.return_value = "Guía de usuário äëïöü l'ecole très Garçon sûr"
        MOCK_DEFINITION = MrBeamDocDefinition(
        ...
        for document in burger_menu_model.documents:
            self.assertEquals(
                document.document_link.language, SupportedLanguage.SPANISH
            )
